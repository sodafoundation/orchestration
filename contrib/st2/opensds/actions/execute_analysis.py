...
#           'HadoopJarStep': {
#               'Jar': 'string'   #example: 's3n://elasticmapreduce/samples/cloudfront/logprocessor.jar' or 'command-runner.jar'
#               'Args':['string']   #example: ['-input', 's3n://mytest/analysis_bucket/input', '-output', 's3n://mytest/analysis_bucket/output'] or ['state-pusher-script']
#           }
#       }
# ]
# }
...
def get_job_end_status(client, jid):
    ...
    while True:
        ...
        status = resp.get("Cluster").get("Status").get("State")
        if status == 'TERMINATED' or status == 'TERMINATED_WITH_ERRORS':
            print("job terminated status: %s" % status)
            break
...
def aws_analysis(args):
    ...
    if final_status != 'TERMINATED':
        print("aws emr: job flow run failed, id=%s, job_status=%s" % (flowid, final_status))
